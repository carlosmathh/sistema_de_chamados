...
export default function TicketsBoardPage() {
  ...
  const title = useMemo(() => {
    return "Board de Tickets (Senior/Engineer)";
  }, []);
  ...
  async function loadBoard() {
    ...
    try {
      ...
      setErrorMsg(detail || "Erro ao carregar board.");
      setResults([]);
    } finally {
      setLoading(false);
    }
    ...
    <div style={{ maxWidth: 1100, margin: "40px auto", fontFamily: "system-ui", padding: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "end", gap: 12 }}>
        <div>
          ...
          <p style={{ margin: "6px 0 0", color: "#555" }}>
            role: <b>{user?.role}</b> • position_team: <b>{user?.position_team}</b>
          </p>
        </div>
      ...
      <div style={{ marginTop: 16, border: "1px solid #ddd", borderRadius: 12, padding: 16 }}>
        ...
        <p style={{ margin: "10px 0 0", color: "#666" }}>
          {loading ? "Carregando..." : `${results.length} ticket(s)`}
        </p>
      </div>

      <div style={{ marginTop: 16 }}>
        {results.length === 0 ? (
          <p style={{ color: "#777" }}>{loading ? "Carregando..." : "Nenhum ticket."}</p>
        ) : (
